...
public class BSTree<T> implements Collection<T>, Iterable<T>, Cloneable,
	...
	private BSTNode<T> findNode(Object item) {
		...
		while (t != null) {
			if(item instanceof Integer){
				orderValue =  ((Integer) item).compareTo((Integer) t.nodeValue);
			}
	...
	/**
	 * Crea un BSTree cloando el pasado por parametro
	 * 
	 * @param tree
	 *            BSTree a clonar
	 */
	...
	private static class BSTNode<T> implements Comparable<T> {
		...
		public int compareTo(T o) {
			...
			return ((Comparable<T>) nodeValue).compareTo(o);
		}
	...
	private String toStringLevel(BSTNode<T> t, int level) {
		...
		if (t != null) {
			if (level == 0)
				...
				aux += toStringLevel(t.left, level - 1)
	...
	/**
	 * Copia el BSTnode pasado por parametro a otro BSTNode
	 * 
	 * @param p
	 *            BSTNode a clonar
	 * @return nuevo BSTNode clonado
	 */
	private BSTNode<T> copy(BSTNode<T> p) {
		return copyTree(p);
	}

	/**
	  * Comprueba que ambos BSTree son iguales
	  */
	@Override
	public boolean equals(Object o) {
		BSTree<T> tree = (BSTree<T>) o;
		Iterator iterador1 = iterator();
		Iterator iterador2 = tree.iterator();
		if (treeSize != tree.treeSize || modCount != tree.modCount)
			return false;
	...
	/**
	 * Elimina todos los elementos del BSTree menos que el valor pasado por parametro
	 * @param i valor que sera tenido en cuenta para eliminar todos los valores del BSTree menores que i
	 */
	public void removeSmaller(int i) {
		Iterator<Integer> iterador =(Iterator<Integer>) iterator();
		while(iterador.hasNext()){
			int element = iterador.next();
			if(element < i)
				iterador.remove();
		}
...
/**
 * Elimina todos los nodos hoja del BSTree
 */
	public void removeLeaves() {
		removeLeavesR(root);
	}
	/**
	 * Comprueba si un nodo es nodo hoja o no, si es lo borra y si no lo es comprueba sus nodos derecho e izquierdo
	 * @param nodo BSTNode a comprobar si es hoja o no
	 */
	private void removeLeavesR(BSTNode<T> nodo){
		if(nodo.left == null && nodo.right == null){
			remove(nodo);
		}else{
			if(nodo.left != null)
				removeLeavesR(nodo.left);
			if(nodo.right != null)
				removeLeavesR(nodo.right);
		}
	...
	/**
	 * Devuelve la suma de todos los nodos menores o iguales que i
	 * @param i valor tope, se sumara el valor de todos los nodos menores o iguales a i
	 * @return suma de nodos menores o iguales que i
	 */
	public int sumOfSmallerThanOrEqualTo(int i) {
		return sumOfSmallerTanOrEqualToR(root,i);
	}
	
	
	private int sumOfSmallerTanOrEqualToR(BSTNode<T> nodo,int i){
		int sum = 0;
		
		Iterator<T> iterador = iterator();
		while(iterador.hasNext()){
			Integer value = (Integer) iterador.next();
			if(value <= i)
				sum = sum + value;
		}
	...
	/**
	 * Devuelve el número de elementos que estan fuera del intervalo cerrado [x,y]
	 * @param x principio del intervalo
	 * @param y final del intervalo
	 * @return Numero de elementos fuera del intevalo
	 */
	public int countOutOfRange(T x, T y) {
		int count = 0;
		
		Iterator<T> iterador = iterator();
		while(iterador.hasNext()){
			Integer value = (Integer) iterador.next();
			if(value < (Integer) x || value > (Integer) y)
				count++;
		}
	...
	/**
	 * Cuenta el numero de nodos entre los niveles i-j, ambos inclusive
	 * @param i nivel desde el que se contaran los nodos
	 * @param j nivel hasta el cual se contaran los nodos
	...
	public int countNodesBetweenLevels(int i, int j) {
		...
		while(i<=j){
			count += toStringLevel(i).split(" ").length;
			i++;
		}
	 ...
	 * @return String con el recorrido en anchura
	 */
	public String toStringBreadthFirstTraversal() {
		String aux = "";
		int level = 0;
		while(level <= height()){
			aux += toStringLevel(level);
			level++;
		}
	...
	/**
	 * Devuelve el recorrido en anchura inverso del arbol
	 * @return String con el recorrido en anchura inverso
	 */
	public String toStringReverseBreadthFirstTraversal() {
		String aux = toStringBreadthFirstTraversal();
