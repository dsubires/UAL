...
public class Heap<T> {
	...
	/**
	 * Asigna el elemento element en el indice i
	 * 
	 * @param i
	 *            indice a insertar elemento
	 * @param element
	 *            elemento a insertar
	 */

	public void assign(int i, T element) {
		if (theHeap.size() == 0 || i > theHeap.size()) {
			theHeap.add(element);
		} else {
			theHeap.add(i, element);
		}
	}

	/**
	 * Comprueba si el heap esta ordenado. Recorrido Top-Down
	 * 
	 * @return true en caso de estar ordenado, false si esta desordenado
	 */
	public boolean isHeap() {
		int parent = 0, child;// = (parent << 1) + 1;
		while (parent < (theHeap.size() >> 2)) {
			child = (parent << 1) + 1;
			if (child < theHeap.size() - 1
					&& compare(theHeap.get(child), theHeap.get(child + 1)) > 0)
				child++; //
	...
	/**
	 * Comprueba si el heap esta ordenado. Recorrido Bottom-Up
	 * 
	 * @return true en caso de estar ordenado, false si esta desordenado
	 */
	...
	/**
	 * Restablece el orden en el heap
	 */
	...
	/**
	 * al elemento en la posicion i, le sumamos element
	 * 
	 * @param i
	...
	/**
	 * al elemento en la posicion i, le restamos element
	 * 
	 * @param iindex
	 *            de nodo heap a modificar
	 * @param element
	...
	/**
	 * eliminta el elemento en el indice i
	 * 
	 * @param i
	...
	public String toString() {
		/*
		 * String aux = "["; Iterator iterador = theHeap.iterator(); while
		 * (iterador.hasNext()) { aux += iterador.next(); if
		 * (iterador.hasNext()) { aux += ", "; } } aux += "]"; return aux;
		 */
	...
	private class HeapDepthFirstIterator implements Iterator<T>{
		...
		public HeapDepthFirstIterator(){
			if(size != 0)
				depthFirstIteratorR(0);
		}
		
		private void depthFirstIteratorR(int i){
			
			if(i < size){
				depthFirstIteratorR(i*2+1);
				al.add(theHeap.get(i));
				depthFirstIteratorR(i*2+2);
			}
		}
