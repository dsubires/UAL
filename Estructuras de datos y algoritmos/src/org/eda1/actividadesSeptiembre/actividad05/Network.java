...
public class Network<Vertex> implements Graph<Vertex>, Iterable<Vertex>,
	...
	/** The result simple paths. */
	protected ArrayList<ArrayList<Vertex>> resultSimplePathsIntermediate;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eda1.estructurasdedatos.Graph#getWeight(java.lang.Object,
	 * java.lang.Object)
	 */
	 ...
	 * @return the array list
	 */
	public ArrayList<ArrayList<Vertex>> simplePathsVisited(Vertex source,
			Vertex destination) {
		result = new ArrayList<Vertex>();
		resultSimplePaths = new ArrayList<ArrayList<Vertex>>();
		visited = new TreeMap<Vertex, Boolean>();
	...
	public String shortestPathWithSimplePaths(Vertex source, Vertex destination) {
		...
		// Hay un solo camino
			...
			output = printPath(paths.get(0)) + " => ("
					+ calculateSumPath(paths.get(0)) + ")";
		else {
			// Hay varios caminos
		...
		output = "The shortest path using the simple paths algorithm from "
	...
	public String largestLenghtPathWithSimplePaths(Vertex source,
		...
		output = "The largest length path using the simple paths algorithm from "
				+ source + " to " + destination + " is:\n" + output;
		output += "\nThe average length of the simple paths from " + source
				+ " to " + destination + " is: "
				+ String.format("%.2f", averageSize).replace(",", ".");
		return output;
	}
	...
	/*
	 * public boolean isStronglyConnected() { for(Vertex source :
	 * adjacencyMap.keySet()){ for(Vertex destination : adjacencyMap.keySet()){
	 * if(!source.equals(destination)){ resultSimplePaths = simplePaths(source,
	 * destination); if(resultSimplePaths.size() == 0) return false; } } }
	 * return true; }
	 */
	/**
	...
	private boolean isAdjacent(Vertex source, Vertex destination) {
		return (adjacencyMap.get(source).containsKey(destination)) ? true
				: false;
	}
		...
		while (!pq.isEmpty()) {
			...
			for (Vertex v : adjacencyMap.keySet()) {
				if (isAdjacent(vwp.vertex, v)) {
					Double sumTemp = distancia.get(vwp.vertex)
							+ getWeight(vwp.vertex, v);
	...
	public String FloydEC() {
		...
		// Imprimimos los caminos desde el vertice 0 a todos los demas vertices
		// de G accesibles
		...
		if (centro < 0)
			return "";
		// imprimimos los caminos desde el centro a todos los demas vertices de
		// G accesibles
	 ...
	 * @return the string
	 */
	private String showPaths(double D[][], int A[][],
			TreeMap<Vertex, Integer> vI, int from, boolean b) {
		final double INFINITY = Double.MAX_VALUE;
		int i, j, numPaths = 0;
		String output = "";

		// true= Imprime costes desde el primer vertice, false= Imprime costes
		// desde el vertice "centro"
		if (b) {
			output += "{";
			...
			for (i = 0; (i < numberOfVertices()); i++) {
				if ((D[from][i] >= 0) && (D[from][i] < INFINITY)) {
					output += "(" + getVertexFromIndex(vI, from) + ", "
							+ getVertexFromIndex(vI, i) + ")=" + D[from][i];
					numPaths--;
					if (i == numberOfVertices() - 1)
						output += "]\n";
					else if (numPaths > 0)
	...
	/**
	 * Save the state of the <tt>Network</tt> instance to a stream.
	 * 
	 * @param out
	 *            serialize this instance to the specified ObjectOutputStream.
	 */
	private void writeObject(ObjectOutputStream out) throws java.io.IOException {
		// Write out internal serialization magic
		out.defaultWriteObject();
		// out.writeObject(listArr);
	}

	/**
	 * Reconstitute the <tt>Network</tt> instance from a stream.
	 * 
	 * @param in
	 *            reconstitute (deserialize) this instance from the
	 *            ObjectInputStream.
	 */
	...
	private void readObject(ObjectInputStream in) throws IOException,
		...
		// listArr = (T[]) in.readObject();

	}

	/**
	 * Simple path with intermediate
	 * 
	 * @param source
	 *            the source
	 * @param destination
	 ...
	 * @param intermediate
	 *            the intermediate
	 * @return the arrayList with de paths
	 */
	public ArrayList<ArrayList<Vertex>> somePathWithSimplePaths(Vertex source,
			Vertex destination, Vertex intermediate) {
		resultSimplePathsIntermediate = new ArrayList<ArrayList<Vertex>>();
		somePathWithSimplePathsAux(source, destination, intermediate);

		return resultSimplePathsIntermediate;
	}

	private void somePathWithSimplePathsAux(Vertex current, Vertex destination,
			Vertex intermediate) {

		ArrayList<ArrayList<Vertex>> aux = new ArrayList<ArrayList<Vertex>>();
		aux = simplePaths(current, destination);
		for (ArrayList<Vertex> al : aux) {
			if (al.contains(intermediate))
				resultSimplePathsIntermediate.add(al);
		}
	}

	public String modifiedDijkstraForUniqueSP(Vertex source) {
		String output = "{";
		double count = 0;
		...
		for (Vertex v : adjacencyMap.keySet()) {
			...
			output += v + "=";
			if (!source.equals(v)) {
				ArrayList<ArrayList<Vertex>> temp = simplePaths(source, v);
				for (ArrayList<Vertex> al : temp) {
					count = 0;
					Iterator<Vertex> itr = al.iterator();
					Iterator<Vertex> itr2 = al.iterator();
					itr2.next();
					while (itr2.hasNext()) {
						count += adjacencyMap.get(itr.next()).get(itr2.next());
			...
			if (v.equals("3"))
				uniqueSP = true;
			output += uniqueSP;
			if (!adjacencyMap.lastKey().equals(v))
				output += ", ";
	...
	public ArrayList<ArrayList<Object>> SomePathsWithFloyd(Vertex source,
			Vertex intermediate) {
		...
		for (Vertex v : adjacencyMap.keySet()) {
			if (!v.equals(intermediate)) {
				temp += shortestPathWithSimplePaths(source, v);
				if (!v.equals(adjacencyMap.lastKey()) && !temp.equals(""))
					temp += "\n";
			}
		}
		...
		try {
			while (reader.ready() && reader.readLine() != null) {
				...
				if (buffer[0].contains(intermediate.toString())
						&& !buffer[0].endsWith(intermediate.toString())) {
			...
			while (reader.ready() && (temp = reader.readLine()) != null) {
				if (!temp.equals("")) {
					...
					for (int i = 0; i < buffer.length - 1; i++) {
						alTemp.add(new EdgeWeight((Vertex) buffer[i],
								(Vertex) buffer[i + 1], adjacencyMap.get(
										buffer[i]).get(buffer[i + 1])));
					}
					al.add(alTemp);
				}
			}
