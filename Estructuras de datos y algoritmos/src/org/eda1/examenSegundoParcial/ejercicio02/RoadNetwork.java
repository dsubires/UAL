...
public class RoadNetwork<Vertex> implements Graph<Vertex>, Iterable<Vertex> {
	...
	public TreeMap<Double, ArrayList<String>> intervalPaths(Vertex from,Vertex to, double dmin, double dmax) {
		...
		Vertex vfrom,vto;
		temp = simplePaths(from, to);
		
		double sumPath;
		for(ArrayList<Vertex> al : temp){
			
			ListIterator<Vertex> iterador = al.listIterator();
			sumPath=0.0;
			while(iterador.hasNext()){
				vfrom = iterador.next();
				if(iterador.hasNext()){
					vto = iterador.next();
					sumPath += adjacencyMap.get(vfrom).get(vto);
					iterador.previous();
				}
			}
			if(sumPath > dmin && sumPath < dmax)
				output.put(sumPath, (ArrayList<String>) al);
		}
