 ...
 */
public class HashAVL<T> implements Collection<T>, Iterable<T> {
	// the hash table
	...
	/**
	 * Aumenta el número de entradas en la tabla Hash, asignando como nuevo tamaño el valor pasado por parametro. 
	 * @param newTableSizeRehash Nuevo tamaño de HashAVL
	 */
	...
	/**
	 * Crea un HashAVL vacio asignando el tamaño y el factor de carga.
	 * @param defaultTableSize tamaño a asignar
	 * @param loadFactor factor de carga a asignar
	 */
...
/*	public int find(Object item) {
		...
		if (table[index] != null) {
			Iterator<T> iter = table[index].iterator();
			T currItem;
			int count = 1;
			while (iter.hasNext()) {
				currItem = iter.next();
...
*/
	/**
	 * Devuelve el número de iteraciones o pasos que recorre hasta llegar a item, -1 si no encuentra item.
	 * @param item item a buscar
	 * @return int con numero de iteraciones/pasos
	 */
	public int find(Object item) {
		//compute the hash table index
		int index = (item.hashCode() & Integer.MAX_VALUE) % table.length;
		return table[index].find2((T) item);
	}
   ...
   public String toString() {
	...
	private class HashAVLTreeIterator implements Iterator<T> {
		...
		Iterator<T> iteratorOfCurrentAVL;	// iterator of the current ArrayList
		...
		HashAVLTreeIterator() {
			...
			if (hashTableSize != 0) {
				...
				while ((i < table.length) && (table[i].isEmpty()))
					i++;
				index = i;
				iteratorOfCurrentAVL = table[index].iterator();
			}
			else {
				index = -1;
				iteratorOfCurrentAVL = null;	
			}
		...
		public T next() {
			...
			lastReturned = iteratorOfCurrentAVL.next();
			
			// move to the next entry in the current AVLTree
			if (!iteratorOfCurrentAVL.hasNext()) {
