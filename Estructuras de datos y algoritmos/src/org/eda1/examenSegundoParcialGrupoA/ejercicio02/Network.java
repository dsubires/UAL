...
public class Network<Vertex> implements Graph<Vertex>, Iterable<Vertex> {
	 ...
	 */
	public static Network<Vertice> readNetwork(String filename)
		...
		// edge weight
		double weight;
		Network<Vertice> net = new Network<Vertice>();
		...
		for (i = 0; i < nVertices; i++){
			Vertice verticeTemp = new Vertice(scaner.next(), Double.parseDouble(scaner.next()));
			verticesAux.add(verticeTemp);
			net.addVertex(verticeTemp);
		}
		// Añadimos aristas
		...
		for (i = 0; i < nEdges; i++) {
			...
			net.addEdge(getVerticeAux(v1,verticesAux), getVerticeAux(v2,verticesAux), weight);
		}
		scaner.close();
		return net;
	}
	
	private static Vertice getVerticeAux(String pueblo,ArrayList<Vertice> listaP){
		for(Vertice v : listaP){
			if(pueblo.equals(v.getPueblo())){
				return v;
			}
		}
		return null;
	...
	*/
	...
	public static Network<Vertice> createNetwork() {
		...
		directorioEntrada = directorioEntrada 
				...
				+ File.separator + "graphFile.txt";		
		
		try {
			return Network.readNetwork(directorioEntrada);
	...
	public TreeMap<Double, String> obtenerResultado() {
		...
		for(Vertice i : (Set<Vertice>) adjacencyMap.keySet()){
			coste = 0.0;
			for(Vertice j : (Set<Vertice>) adjacencyMap.keySet()){
				if(!i.equals(j))
						coste += j.getPoblacion() * adjacencyMap.get(i).get(j);
			}
			mapa.put(coste, i.getPueblo());
		}
		return mapa;
	}
